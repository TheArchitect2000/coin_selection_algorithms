...
for i in range(1,iterations):
    ...
    pay = 0
    num = 0
    for i in range(len(utxos)-1,0,-1):
        if utxos[i] <= rem and rem > 0:
            num = num + 1
            pay = pay + utxos[i]
            del utxos[i]
            rem = rem - pay
...
plt.show()
fig.savefig(f'Plots/Normal-Greedy_{datetime.datetime.now().strftime("%d-%m-%Y--%H-%M-%S")}.jpg', format="jpg")
