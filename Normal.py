...
def BnBRecursion(d,currentSelection, rounds):
    rounds = rounds - 1
    utxosSorted = [u for u in utxos[m] if u not in currentSelection]
...
for it in range(0, iterations):
    ...
    deposit = np.random.normal(1000, 250, numDeposits)
    ...
    utxos[m] = [u for u in utxos[m] if u not in Selected]
    utxos[m].sort(reverse=True)
    while rem > 0:
        num = num + 1
        minUtxo = utxos[m].pop()
    ...
    if len(Selected) < len(bestSet):
        num = num + 1
    utxos[m] = [u for u in utxos[m] if u not in bestSet]
    pay = sum(bestSet)
    ...
    utxos[m] = [u for u in utxos[m] if u not in currentSelection]
...
# Saving the objects:
with open('var6.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    ...
    for j in range(2):
        ...
        while cond:
            try:
                ...
                ax1[i,j].set_xlim([0, maxUtxo_old])
